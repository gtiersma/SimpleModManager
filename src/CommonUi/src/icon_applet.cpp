...
IconApplet::IconApplet(View* contentView)
    : IconApplet::IconApplet()
{
...
void IconApplet::setIconFromFile(std::string path)
{
    if (path.empty())
    {
...
void IconApplet::setIconFromRes(std::string path)
{
    if (path.empty())
    {
        this->icon->setVisibility(Visibility::GONE);
    ...
    {
        this->icon->setVisibility(Visibility::VISIBLE);
        this->icon->setImageFromRes(path);
    }
}

void IconApplet::setIconFromMem(const unsigned char* data, int size)
{
    this->icon->setVisibility(Visibility::VISIBLE);
    this->icon->setImageFromMem(data, size);
}

void IconApplet::setHeaderVisibility(Visibility visibility)
{
    header->setVisibility(visibility);
}

void IconApplet::setFooterVisibility(Visibility visibility)
{
    footer->setVisibility(visibility);
}

void IconApplet::setTitle(std::string title)
{
    this->title->setText(title);
}

void IconApplet::pushContentView(View* view)
{
...
void IconApplet::popContentView(std::function<void(void)> cb)
{
...
void IconApplet::setContentView(View* view)
{
    if (this->contentView)
    {
        // Remove the node
...
void IconApplet::handleXMLElement(tinyxml2::XMLElement* element)
{
    if (this->contentView)
...
View* IconApplet::create()
{
    return new IconApplet();
}
